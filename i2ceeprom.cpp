...
cI2CEEProm::cI2CEEProm(QString dn, int dl, ushort adr) {
    DevNode = dn;
    DebugLevel = dl;
    I2CAdress = adr;
}
...
int c24LC256::WriteData(char* data,ushort n,ushort adr) {
    uchar outpBuf[66]; // 2 adr byte, max 64 byte daten
...
int c24LC256::ReadData(char* data,ushort n,ushort adr) {
    uchar outpBuf[2];
    uchar inpBuf[n];
