// implementation des eigenlichen servers com5003(d)
...
static sRange RangeUL1[Ux_nr]=
{
    {"480V",    "480.0",    "4712563",  0,  Volt,   rngPhys, NULL},
    {"240V",    "240.0",    "4712563",  0,  Volt,   rngPhys, NULL},
    {"120V",    "120.0",    "4712563",  0,  Volt,   rngPhys, NULL},
    { "60V",     "60.0",    "4712563",  0,  Volt,   rngPhys, NULL},
    { "15V",     "15.0",    "4859831",  0,  Volt,   rngPhys, NULL},
    {  "5V",      "5.0",    "4516206",  0,  Volt,   rngPhys, NULL}
};

static sRange RangeUL2[Ux_nr]=
{
    {"480V",    "480.0",    "4712563",  0,  Volt,   rngPhys, NULL},
    {"240V",    "240.0",    "4712563",  0,  Volt,   rngPhys, NULL},
    {"120V",    "120.0",    "4712563",  0,  Volt,   rngPhys, NULL},
...
static sRange RangeIL1[Ix_nr]=
{
    {"160A",    "160.0",    "5005789",  0,  Ampere, rngPhys, NULL},
    {"100A",    "100.0",    "4692928",  0,  Ampere, rngPhys, NULL},
    { "50A",     "50.0",    "4692928",  0,  Ampere, rngPhys, NULL},
    { "25A",     "25.0",    "4692928",  0,  Ampere, rngPhys, NULL},
    { "10A",     "10.0",    "4692928",  0,  Ampere, rngPhys, NULL},
    {"2.5A",      "2.5",    "4692928",  0,  Ampere, rngPhys, NULL},
    {"1.0A",      "1.0",    "4692928",  0,  Ampere, rngPhys, NULL},
    {"500mA",     "0.5",    "4692928",  0,  Ampere, rngPhys, NULL},
    {"250mA",    "0.25",    "4692928",  0,  Ampere, rngPhys, NULL},
    {"100mA",     "0.1",    "4692928",  0,  Ampere, rngPhys, NULL},
    { "50mA",    "0.05",    "4692928",  0,  Ampere, rngPhys, NULL},
    { "25mA",   "0.025",    "4692928",  0,  Ampere, rngPhys, NULL},
    { "10mA",    "0.01",    "4692928",  0,  Ampere, rngPhys, NULL},
    {  "5mA",   "0.005",    "4692928",  0,  Ampere, rngPhys, NULL}
};


static sRange RangeIL2[Ix_nr]=
{
    {"160A",    "160.0",    "5005789",  0,  Ampere, rngPhys, NULL},
    {"100A",    "100.0",    "4692928",  0,  Ampere, rngPhys, NULL},
    { "50A",     "50.0",    "4692928",  0,  Ampere, rngPhys, NULL},
...
cCom5003Server::cCom5003Server()
    ...
    MeasChannelList << "UL1" << "UL2" << "UL3" << "IL1" << "IL2" << "IL3";
    ...
    {
        UL1RangeList << sr->RName;
        sr->pJustData=new cCOM5003JustData; // default justage werte
    }

    sr=&RangeUL2[0];
    for (unsigned int i = 0; i<(sizeof(RangeUL2)/sizeof(sRange)); i++,sr++)
    {
        UL2RangeList << sr->RName;
        sr->pJustData=new cCOM5003JustData; // default justage werte
    }

    sr=&RangeUL3[0];
    for (unsigned int i = 0; i<(sizeof(RangeUL3)/sizeof(sRange)); i++,sr++)
    {
        UL3RangeList << sr->RName;
        sr->pJustData=new cCOM5003JustData; // default justage werte
    }

    sr=&RangeIL1[0];
    for (unsigned int i = 0; i<(sizeof(RangeIL1)/sizeof(sRange)); i++,sr++)
    {
        IL1RangeList << sr->RName;
        sr->pJustData=new cCOM5003JustData; // default justage werte
    }

    sr=&RangeIL2[0];
    for (unsigned int i = 0; i<(sizeof(RangeIL2)/sizeof(sRange)); i++,sr++)
    {
        IL2RangeList << sr->RName;
        sr->pJustData=new cCOM5003JustData; // default justage werte
    }

    sr=&RangeIL3[0];
    for (unsigned int i = 0; i<(sizeof(RangeIL3)/sizeof(sRange)); i++,sr++)
    {
        IL3RangeList << sr->RName;
        sr->pJustData=new cCOM5003JustData; // default justage werte
    }

    ChannelRangeListMap["UL1"] = &UL1RangeList;
    ChannelRangeListMap["UL2"] = &UL2RangeList;
    ChannelRangeListMap["UL3"] = &UL3RangeList;
    ChannelRangeListMap["IL1"] = &IL1RangeList;
    ChannelRangeListMap["IL2"] = &IL2RangeList;
    ChannelRangeListMap["IL3"] = &IL3RangeList;

    ChannelCValueListMap["UL1"] = &CValueList; // die korrektur für die sensorik ist für alle kanäle gleich
    ChannelCValueListMap["UL2"] = &CValueList;
    ChannelCValueListMap["UL3"] = &CValueList;
    ChannelCValueListMap["IL1"] = &CValueList;
    ChannelCValueListMap["IL2"] = &CValueList;
    ChannelCValueListMap["IL3"] = &CValueList;

    CCoeffientList << "GCC0" << "GCC1" << "PCC0" << "PCC1" << "PCC2" << "PCC3" << "OCC0";  // wir brauchen hier nur eine für alle kanäle
    ChannelCCoeffientListMap["UL1"] = &CCoeffientList; // s.o. alle gleich
    ChannelCCoeffientListMap["UL2"] = &CCoeffientList;
    ChannelCCoeffientListMap["UL3"] = &CCoeffientList;
    ChannelCCoeffientListMap["IL1"] = &CCoeffientList;
    ChannelCCoeffientListMap["IL2"] = &CCoeffientList;
    ChannelCCoeffientListMap["IL3"] = &CCoeffientList;
    
    CNodeList << "GCN0" << "GCN1" << "PCN0" << "PCN1" << "PCN2" << "PCN3" << "OCN0";    
    ChannelCNodeListMap["UL1"] = &CNodeList; // s.o. alle gleich
    ChannelCNodeListMap["UL2"] = &CNodeList;
    ChannelCNodeListMap["UL3"] = &CNodeList;
    ChannelCNodeListMap["IL1"] = &CNodeList;
...
int cCom5003Server::sDebugLevel(char* s) {
    QString p = s;
    int dl = p.toInt();
...
int cCom5003Server::SetI2CDevNode(char* s) {
    sI2CDevNode=s;
    return(0); // immer ok ?
}


int cCom5003Server::SetI2CSlaveAdr(char* s) {
    bool ok=true;
...
int cCom5003Server::SetI2CMasterAdr(char* s) {
    bool ok=true;
    int r=1; // erstmal falsch
    short adr= QString(s).toShort(&ok);
...
int cCom5003Server::I2CBootloaderCommand(bl_cmd* blc) {
    int rlen = -1; // rückgabewert länge ; < 0 bedeutet fehler
    uchar inpBuf[5]; // die antwort auf command ist immer 5 bytes lang
...
int cCom5003Server::I2CWriteCommand(hw_cmd* hc) {
    int rlen = -1; // rückgabewert länge ; < 0 bedeutet fehler
...
int cCom5003Server::I2CReadOutput(quint8* data, ushort dlen) {
    int rlen = -1; // rückgabewert länge ; < 0 bedeutet fehler
...
void  cCom5003Server::GenCommand(hw_cmd* hc) {
    short len = 6 + hc->plen;
...
void  cCom5003Server::GenBootloaderCommand(bl_cmd* blc) {
...
char* cCom5003Server::GenAdressPointerParameter(uchar adresspointerSize, ulong adr)
{
...
bool cCom5003Server::ReadJustData() {
    ...
    while (!ba2stream.atEnd()) {
	    ...
	    sRange dummy;
        dummy.pJustData=new cCOM5003JustData; // default justage werte
...
sRange* cCom5003Server::SearchRange(QString& ch,QString& rng) { // holt einen zeiger auf sRange abhängig v. kanal,range
...
bool cCom5003Server::GetAdjInfo(QDomNode n)
{ // n steht auf einem element dessen tagname channel ist
    typedef QMap<QString,cCOM5003JustData*> tRangeJustMap; // zur temporären speicherung von justagedaten
    ...
    for (uint i=0; i<nl.length(); i++) {
        ...
        if (tName=="Name") {
            ...
            sl=it.data();
            for (it2 = sl->begin(); it2 != sl->end() ; it2++) RangeJustMap[*it2]=new cCOM5003JustData(); // default daten bei import
        } else
            if (tName=="Range") {
                    ...
                    if (tName2=="Name") {
                        ...
                        if (tName2=="Status") {
                            ...
                            if (it4 != RangeJustMap.end() ) { // den bereich gibts
                                cCOM5003JustData* jwmd=it4.data();
...
const char* cCom5003Server::mSetSampleMode(char* s) {
    bool ok;
    QString par = pCmdInterpreter->m_pParser->GetKeyword(&s); // holt den parameter aus dem kommando
    int sm = par.toInt(&ok);
...
const char* cCom5003Server::mSetSampleFrequency(char* s) {
    bool ok;
    QString par = pCmdInterpreter->m_pParser->GetKeyword(&s); // holt den parameter aus dem kommando
...
const char* cCom5003Server::mFile2Justdata(char* s) {
   ...
   if  (TheDocType.name() != "COM5003AdjustmentData") {
...
const char* cCom5003Server::mJustData2File(char* s) {
    QDomDocument justdata( "COM5003AdjustmentData" );
...
const char* cCom5003Server::mEEProm2JustData(char* s) {
...
const char* cCom5003Server::mJustData2EEProm(char* s) {
     if (pCmdInterpreter->m_pParser->GetChar(&s) ) { // schau mal nach ob noch was kommt
	Answer = NACKString; // not acknowledge 	    
	return Answer.latin1();
    }
...
const char* cCom5003Server::mGetEEpromEnable() {
    if ( EEPromAccessEnable() )
	Answer="1";
    else
	Answer="0";
...
const char* cCom5003Server::mGetEEPrChksum() {
    Answer = QString("0x%1").arg(m_nChksumFlash,0,16);
    return Answer.latin1();
}


const char* cCom5003Server::mGetPSamples() {
    char PAR[1];
...
const char* cCom5003Server::mGetSampleFrequency() {
    char PAR[1];
...
const char* cCom5003Server::mControlerMemUpdate(bl_cmdcode blwriteCmd,char* s)
{
	...
	if ( true  ) // bootloader läuft hoffentlich schon ...
	{
		...
		{ // wir haben die benötigten informationen vom bootloader
		    ...
		    {
			char* adrParameter;
            ushort adrParLen = BootloaderInfo.AdressPointerSize;
			...
			{ // wir sind die adresse los geworden
                ...
                ushort memlen = MemByteArray.count();
...
const char* cCom5003Server::mControlerFlashUpdate(char* s)
{
    return mControlerMemUpdate(blWriteFlashBlock, s);
}


const char* cCom5003Server::mControlerEEpromUpdate(char* s)
{
    return mControlerMemUpdate(blWriteEEPromBlock, s);
}


const char* cCom5003Server::mControlerStartBootloader(char* s)
{
     if (pCmdInterpreter->m_pParser->GetChar(&s) ) { // schau mal nach ob noch was kommt
	Answer = NACKString; // not acknowledge 	    
	return Answer.latin1();
...
const char* cCom5003Server::mControlerStartProgram(char* s)
{
    if (pCmdInterpreter->m_pParser->GetChar(&s) ) { // schau mal nach ob noch was kommt
	Answer = NACKString; // not acknowledge 	    
	return Answer.latin1();
...
const char* cCom5003Server::mGetText(hw_cmdcode hwcmd) {
    char PAR[1];
    int rlen;
...
const char* cCom5003Server::mGetSerialNumber() {
    return mGetText(hwGetSerialNr);
}


const char* cCom5003Server::mSetPCBVersion(char* s) {
   QString spar = pCmdInterpreter->m_pParser->GetKeyword(&s); // holt den parameter aus dem kommando   
    ushort slen = spar.length();
...
const char* cCom5003Server::mGetPCBVersion() {
    return mGetText(hwGetPCBVersion);
};


const char* cCom5003Server::mGetCTRLVersion() {
    return mGetText(hwGetCtrlVersion);
}


const char* cCom5003Server::mGetLCAVersion() {
    return mGetText(hwGetLCAVersion);
}


const char* cCom5003Server::mGetI2CMasterAdress() {
    Answer = QString::number(I2CMasterAdr);
    return Answer.latin1();      
}


const char* cCom5003Server::mGetI2CSlaveAdress() {
    Answer = QString::number(I2CSlaveAdr);
    return Answer.latin1();          
}


const char* cCom5003Server::mGetI2CDeviceNode() {
    return sI2CDevNode.latin1();      
}
 

const char* cCom5003Server::mSetDebugLevel(char* s) {
    bool ok;
    QString par = pCmdInterpreter->m_pParser->GetKeyword(&s); // holt den parameter aus dem kommando
    int dl=par.toInt(&ok);
...
const char* cCom5003Server::mGetDebugLevel() {
    Answer = QString::number(DebugLevel);
    return Answer.latin1();          
}
 

const char* cCom5003Server::mGetDeviceVersion() {
...
const char* cCom5003Server::mGetServerVersion() {
    return sSoftwareVersion.latin1();
}


const char* cCom5003Server::mGetAdjustmentStatus() {
    bool adjusted = true;
    int adj;
...
const char* cCom5003Server::mGetChannelStatus() {
...
const char* cCom5003Server::mOutCValueCatalog() {
    Answer="";
...
const char* cCom5003Server::mGetCValueCCoefficientName() {
//    QString  dedicatedChannel = pCmdInterpreter->dedicatedList.first();
...
const char* cCom5003Server::mGetCValueCNodeName() {
//    QString  dedicatedChannel = pCmdInterpreter->dedicatedList.first();
    pCmdInterpreter->dedicatedList.pop_front();
    QString dedicatedCValue = pCmdInterpreter->dedicatedList.first();
...
bool cCom5003Server::EEPromAccessEnable() {
    char PAR[1];
...
void cCom5003Server::DelClient(int s) { // entfernt einen client
...
bool cCom5003Server::Test4HWPresent() {
    return( sSerialNumber != "Unknown");
}


const char* cCom5003Server::mSetCValueCCoefficient(char* s)
{
bool ok;
    QString par = pCmdInterpreter->m_pParser->GetKeyword(&s); // holt den parameter aus dem kommando
...
const char* cCom5003Server::mGetCValueCCoefficient()
{
    QString dedicatedChannel = pCmdInterpreter->dedicatedList.first();
    pCmdInterpreter->dedicatedList.pop_front();
    QString dedicatedRange = pCmdInterpreter->dedicatedList.first();
...
const char* cCom5003Server::mSetCValueCNode(char* s)
{
bool ok1, ok2;
    QString par = pCmdInterpreter->m_pParser->GetKeyword(&s); // holt den parameter aus dem kommando
...
const char* cCom5003Server::mGetCValueCNode()
{
    QString dedicatedChannel = pCmdInterpreter->dedicatedList.first();
    pCmdInterpreter->dedicatedList.pop_front();
    QString dedicatedRange = pCmdInterpreter->dedicatedList.first();
...
const char* cCom5003Server::mCmpCCoefficient(char*)
{
   QString dedicatedChannel = pCmdInterpreter->dedicatedList.first();
...
const char* cCom5003Server::mSetStatus(char* s) {
    bool ok;
    QString par = pCmdInterpreter->m_pParser->GetKeyword(&s); // holt den parameter aus dem kommando
    int st=par.toInt(&ok);
    if (ok) {
...
const char* cCom5003Server::mGetStatus() {
    QString dedicatedChannel = pCmdInterpreter->dedicatedList.first();
    pCmdInterpreter->dedicatedList.pop_front();
    QString dedicatedRange = pCmdInterpreter->dedicatedList.first();
    sRange* ranges=SearchRange(dedicatedChannel,dedicatedRange);
...
const char* cCom5003Server::mGetCValue(char* s) { // abfrage des korrekturwertes (ev. mit parameter)
    bool ok;
    QString par = pCmdInterpreter->m_pParser->GetKeyword(&s); // holt den parameter aus dem kommando
    QString dedicatedChannel = pCmdInterpreter->dedicatedList.first();
    pCmdInterpreter->dedicatedList.pop_front();
...
const char* cCom5003Server::mGetRejection() {
    QString dedicatedChannel = pCmdInterpreter->dedicatedList.first();
    pCmdInterpreter->dedicatedList.pop_front();
    QString dedicatedRange = pCmdInterpreter->dedicatedList.first();
    sRange* ranges=SearchRange(dedicatedChannel,dedicatedRange);
...
const char* cCom5003Server::mGetRValue() {
    QString dedicatedChannel = pCmdInterpreter->dedicatedList.first();
    pCmdInterpreter->dedicatedList.pop_front();
    QString dedicatedRange = pCmdInterpreter->dedicatedList.first();
    sRange* ranges=SearchRange(dedicatedChannel,dedicatedRange);
...
const char* cCom5003Server::mOutRangeCatalog() {
    QString dedicatedChannel = pCmdInterpreter->dedicatedList.first();
    Answer="";
    QTextOStream* os=new QTextOStream(&Answer);
    tChannelListMap::Iterator it=ChannelRangeListMap.find(dedicatedChannel);
...
const char* cCom5003Server::mSetRange(char* s) {
    char dnr;
...
const char* cCom5003Server::mSetCalculateModel() {
    QString dedicatedChannel = pCmdInterpreter->dedicatedList.first();
...
const char* cCom5003Server::mOutChannelCatalog() {
    Answer="";
    QTextOStream* os=new QTextOStream(&Answer);
...
int cCom5003Server::Execute() { // server ausführen
    int sock;
...
void cCom5003Server::AddClient(int s, sockaddr_in* addr) { // fügt einen client hinzu
    clientlist.append(new cZHClient(s,addr));
    if DEBUG3 syslog(LOG_INFO,"client %d added\n",s);
}


const char* cCom5003Server::SCPICmd( SCPICmdType cmd, char* s) {
    switch ((int)cmd)	{
...
const char* cCom5003Server::SCPIQuery( SCPICmdType cmd, char* s) {
    switch ((int)cmd)	{
